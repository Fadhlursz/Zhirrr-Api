...
var creatorList = ['@sul.lur','@Fadlur','@Fadhlur','@sul.lur', '@follow ig gw :(','@sul.lur','@follow napa'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6281395771492 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/6281395771402 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------Fadhlur'+'FADHLUR--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'apidlur') return res.json(loghandler.invalidKey)
...
router.get('/remove', (req, res, next) => {
    ...
    if (!apikey) return res.json(loghandler.notparam)
    if (!(status && apikeyInput && email && nomorhp && name && age && country && exp)) return res.json(loghandler.notAddApiKey)
    if (apikey != 'rulgans') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'rulgans') return res.json(loghandler.invalidKey)
